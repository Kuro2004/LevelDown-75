...
commandObj.onTrigger = function(player, target, rank)
    ...
    if target == nil then
        error(player, "No target specified.")
        return
    end
    ...
    rank = tonumber(rank)
    if rank == nil or rank < 2 or rank > 9 then
        error(player, "You must enter a valid rank number (must be 2-9).")
        return
    end
    ...
    if rank >= 2 then
        ...
        targ:addKeyItem(xi.ki.LIGHT_OF_DEM)
        targ:addKeyItem(xi.ki.LIGHT_OF_HOLLA)
        targ:addKeyItem(xi.ki.LIGHT_OF_MEA)
    ...
    if rank >= 6 then
        ...
        targ:addKeyItem(xi.ki.LIGHT_OF_VAHZL)
    ...
    if rank >= 8 then
        targ:addKeyItem(xi.ki.LIGHT_OF_ALTAIEU)
