...
if mob:getMainLvl() > 74 and mob:getMainLvl() <= 80 then
        ...
        mob:addMod(xi.mod.MEVA, 15)
        mob:addMod(xi.mod.HASTE_MAGIC, 10)
elseif mob:getMainLvl() > 80 and mob:getMainLvl() <= 89 then
        mob:addMod(xi.mod.ATT, 140)
        mob:addMod(xi.mod.DEF, 25)
        mob:addMod(xi.mod.ACC, 70)
...
elseif mob:getMainLvl() > 89 and mob:getMainLvl() <= 99 then
        mob:addMod(xi.mod.ATT, 180)
        mob:addMod(xi.mod.DEF, 35)
        mob:addMod(xi.mod.ACC, 90)
        mob:addMod(xi.mod.EVA, 25)
        mob:addMod(xi.mod.MATT, 15)
        mob:addMod(xi.mod.MDEF, 35)
...
m:addOverride(string.format("xi.zones.%s.mobs.%s.onMobDeath", entry[2], entry[1]), function(mob, player, params, npc)
        ...
        local varib = player:getCharVar(varName)
              ...
              -- if varib >= 0 
              --  then
        local randb = math.random(1,2) * entry[3]
